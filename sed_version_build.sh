...
# version in loginModal.partial.html
sed -i "s/version: .*/version: ${ZABBIX_BULLSEQUANA_EDGE_VERSION}/" Dockerfiles/loginModal.partial.html

# version in zabbix templates
sed -i "s#<description>.*</description>#<description>${ZABBIX_BULLSEQUANA_EDGE_VERSION}</description>#" zabbix/server/externalscripts/template-atos_openbmc-lld-zbxv4.xml
sed -i "s#<description>.*</description>#<description>${ZABBIX_BULLSEQUANA_EDGE_VERSION}</description>#" zabbix/server/externalscripts/template-atos_openbmc-host-zbxv4.xml
sed -i "s#<description>.*</description>#<description>${ZABBIX_BULLSEQUANA_EDGE_VERSION}</description>#" zabbix/server/externalscripts/template-atos_openbmc-rsyslog-zbxv4.xml
